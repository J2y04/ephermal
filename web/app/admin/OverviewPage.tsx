...
function buildMockRevenue(): RevenueData {
  const counts = [1, 2, 1, 0, 3, 2, 4, 1, 0, 2, 1, 3, 2, 4, 1, 2, 3, 1, 0, 2, 1, 4, 2, 3, 1, 2, 0, 3, 2, 4];
  const signups = Array.from({ length: 30 }, (_, i) => {
    const d = new Date(MOCK_BASE_TIME - (29 - i) * 86_400_000).toISOString().slice(0, 10);
    return { date: d, count: counts[i] };
  });
  return { signups, generated_at: new Date(MOCK_BASE_TIME).toISOString() };
}
...
function buildMockUsers(): UsersData {
  ...
  const users: SignupUser[] = emails.map((email, i) => ({
    id: `user_mock${i}`,
    email,
    created_at: new Date(MOCK_BASE_TIME - MOCK_SIGNUP_OFFSETS_MS[i]).toISOString(),
    banned: i === 1,
    plan: plans[i],
  }));
...
export default function AdminOverviewPage() {
  ...
  }, [localPreview, previewRevenue, previewUsers, session]);
  ...
  }, [activeRevenue, signups7d, signups7dPrev]);

  const bannedCount = activeUsersList.filter(u => u.banned).length;

  const recentSignups = useMemo(
    () => [...activeUsersList].sort((a, b) => (a.created_at < b.created_at ? 1 : -1)).slice(0, 6),
    [activeUsersList],
  );
    ...
    <div className="mx-auto max-w-[1560px] px-8 py-8 lg:px-10 lg:py-10">
        ...
        {activeLoading ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-[152px] animate-pulse rounded-[24px] bg-eph-surface2" />
            ))}
          </div>
        ) : (
          <SectionCards
      ...
      <div className="mt-6 grid grid-cols-12 gap-5">
        ...
        <div className="col-span-12 xl:col-span-4">
          <Squircle cornerRadius={28} className="widget-shadow flex h-full flex-col border border-eph-border bg-eph-surface p-6">
            <div className="flex items-center justify-between">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-eph-subtle">Recent signups</div>
              {!activeLoading && activeUsersList.length > 0 && (
                <span className="text-xs text-eph-subtle">{activeUsersList.length} total</span>
              )}
            </div>
